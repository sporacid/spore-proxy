...
namespace spore::proxies::tests::tu
{
    ...
    template std::size_t some_work(const shared_proxy<facade<proxy_dispatch_dynamic<>>>& proxy);
    template std::size_t some_work(const shared_proxy<facade<proxy_dispatch_static<>>>& proxy);
}